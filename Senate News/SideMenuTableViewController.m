...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        return [self setupViewForSectionHeader:@"ព័ត៌មានសំខាន់"];
    } else if (section == 1){
...
-(UIView *)setupViewForSectionHeader:(NSString *)text {
    ...
    if([text isEqualToString:@"ព័ត៌មានសំខាន់"]){
        imageView.image = [UIImage imageNamed:@"VIP-64.png"];
    }else if ([text isEqualToString:@"ប្រតិទិនប្រជំុ"]) {
